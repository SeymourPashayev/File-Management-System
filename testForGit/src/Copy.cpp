...
void Copy::copy() {
    ...
    case Copy::COPY_ITEMS::FILES: {
        ...
        case Copy::COPY_SELECTIVITY::CUSTOM: {
            std::deque<int> selectedFiles{ selectFiles() };
            long long count{ 1 };
...
std::deque<int> Copy::selectFiles() {
...
void Copy::displayFiles(std::vector<int>& allFiles, std::deque<int>& selectedFiles, std::ostringstream& itemLstOutput) {
    ...
    while (std::cin >> selectedID && selectionCount < allFiles.size()) {
        if (std::binary_search(allFiles.begin(), allFiles.end(), selectedID) && !std::binary_search(selectedFiles.begin(), selectedFiles.end(), selectedID)) {
            ...
            std::cerr << std::endl << "Invalid input. Make sure file ID is on the list and the last entry is not a duplicate." << std::endl << std::endl;
        }
    }
